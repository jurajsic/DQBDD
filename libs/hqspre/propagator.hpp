...
class ConflictException : public std::exception
{
   public:
     ...
     */
    ~ConflictException() noexcept override = default;
};
 ...
 *
 * This class implements the two-watch-literal scheme for SAT for efficient unit
 * literal detection. In contrast to QBF-based BCP, SAT-based BCP does not
 * perform universal reduction to detect more unit literals.
 */
class SatPropagator
{
   public:
    SatPropagator(const Prefix* const prefix, const std::vector<Clause>& clauses,
    ...
    virtual ~SatPropagator() noexcept         = default;
    ...
    /**
     * \brief Returns a reference to the list of found unit clauses (including the
     * initial assumptions)
     */
   ...
   private:
    ...
    /** \brief Assignment for the found unit literals (provided there was no
     * conflict) */
    ...
    /** \brief Watch literal datastructure. _watches[lit] contains the IDs of all
     * clauses in which `lit` is watched. */
...
{
   public:
    QbfPropagator(const Prefix* const prefix, const std::vector<Clause>& clauses,
                  const std::vector<std::unordered_set<BinaryClause>>& implications) noexcept :
        SatPropagator(prefix, clauses, implications)
    {}

    ~QbfPropagator() noexcept override = default;
