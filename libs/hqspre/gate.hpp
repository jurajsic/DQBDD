...
class Gate
{
   public:
     ...
     *
     * \note Gates with no encoding clauses are considered invalid. Gates with
     * type UNKNOWN can be valid; they may result from semantic gate detection.
     */
    Gate() = default;
    ...
    Gate(GateType type, Literal output, const std::initializer_list<Literal>& inputs,
         const std::initializer_list<ClauseID>& clauses) :
        _type(type),
        _output_literal(output),
        _input_literals(inputs),
        _encoding_clauses(clauses)
    ...
    GateType              _type{GateType::UNKNOWN};  ///< Type of the gate (AND/XOR)
    Literal               _output_literal{0u};       ///< output literal (might be negated for NAND/EQUIV)
...
class GateInfo
{
   public:
     ...
     *
     * This increases the size of the clause-related internal data structures in
     * the gate handler appropriately.
     */
     ...
     *
     * This increases the size of the variable-related internal data structures in
     * the gate handler appropriately.
     */
    ...
    /**
     * \brief Returns true iff the given variable is defined by a gate in the
     * database \note A return value of true does not necessarily mean that the
     * corresponding gate is still valid. An encoding clause might have been
     * modified in the mean time. (If the gate was invalidated by a call to
     * GateInfo::invalidateGate(Gate&), then this function takes that into account
     * and returns false for the gate output).
     */
     ...
     *       is still valid. An encoding clause might have been modified in the
     * mean time. (If the gate was invalidated by a call to
     * GateInfo::invalidateGate(Gate&), then this function takes that into account
     * and returns the correspondingly decreased value).
     */
    ...
    /**
     * \brief Returns true iff the given clause is used in the definition of a
     * gate in the database \note A return value of true does not necessarily mean
     * that the corresponding gate is still valid. An encoding clause might have
     * been modified in the mean time. (If the gate was invalidated by a call to
     * GateInfo::invalidateGate(Gate&), then this function takes that into account
     * and returns false for the encoding clause).
     */
   ...
   private:
    ...
    std::vector<bool> _clause_modified;     ///< \brief Which clauses have been modified since the
                                            ///< last call to GateInfo::update()
    std::vector<bool> _gate_clause;         ///< \brief Is a clause used to encode a gate
                                            ///< in the database
    std::vector<bool> _gate_output;         ///< \brief Is a variable the output of a gate
                                            ///< in the database
    std::vector<unsigned int> _gate_input;  ///< \brief For each variable the number of gate the variable
                                            ///< is an input of
};
