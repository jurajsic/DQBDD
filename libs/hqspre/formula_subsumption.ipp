 ...
 * `short_clause` is contained in the formula (otherwise set `c_nr` to a
 * negative value).
 *
 * \param short_clause the clause for which we check if it subsumes any other
 * clause \param c_nr the ID of `short_clause` if it is part of the formula, -1
 * otherwise \return the number of deleted clauses \sa
 * Formula::binarySubsumption() \sa Formula::narySubsumption()
 */
inline std::size_t
Formula::isBackwardSubsuming(const Clause& short_clause, const int c_nr, bool delete_subsumed)
{
