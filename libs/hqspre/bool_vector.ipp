...
inline std::size_t
BoolVector::size() const noexcept
{
    return _size;
}

inline bool
BoolVector::uninitialized() const noexcept
{
    return _bins == nullptr;
}

inline bool
BoolVector::get(const std::size_t index) const
{
    return (_bins[(index / BinSize)] >> (index % BinSize)) & 1ul;
}

inline void
BoolVector::setBin(const std::size_t index, BoolVector::BinType b)
{
    _bins[index] = b;
}
