 ...
 */
std::pair<int, int>
Formula::computeExpansionCosts(const Variable uvar) const
{
...
void
Formula::markTransitiveUnits(std::stack<Literal>& units, std::vector<bool>& marked) const
{
    while (!units.empty()) {
...
Formula::computeExpansionCosts2(const Literal ulit, const std::set<Variable>& pseudo_deps)
{
    ...
    for (ClauseID c_nr = 0; c_nr < _clauses.size(); ++c_nr) {
        ...
        for (const Literal lit : _clauses[c_nr]) {
            ...
            // we have to copy each clause where an existential literal exists which
            // depends on "uvar"
...
/**
 * \brief Applies universal expansion to those universal variables which can
 * cheaply be eliminated. \return True if the formula was modified.
 */
bool
Formula::applyUniversalExpansion()
{
    VLOG(1) << __FUNCTION__;
...
bool
Formula::applyUniversalExpansion2()
{
    VLOG(1) << __FUNCTION__;
    ...
    if (_prefix->type() == PrefixType::QBF) {
        ...
        while (true) {
            ...
            for (auto level = static_cast<int>(_qbf_prefix->getMaxLevel()); level >= 0; --level) {
                // Exit routine if we exceed our maximal expansion size
...
Formula::computeVarElimSet()
{
    ...
    for (Variable var1 = minVarIndex(); var1 < maxVarIndex(); ++var1) {
        ...
        for (Variable var2 = var1 + 1; var2 <= maxVarIndex(); ++var2) {
            ...
            two_sided_difference(dep1.cbegin(), dep1.cend(), dep2.cbegin(), dep2.cend(),
...
bool
Formula::applyUniversalExpansionDQBF()
{
    ...
    VLOG(2) << __FUNCTION__ << ": We need to eliminate " << to_eliminate.size() << " univ vars out of " << numUVars()
            << ": " << to_eliminate;
    ...
    while (!to_eliminate.empty()) {
        ...
        if (next_pos >= 0
            && (force_eliminate || to_eliminate.size() <= 3
                || static_cast<double>(min_cost) < 0.1 * static_cast<double>(old_size))) {
    ...
    if (_prefix->type() == PrefixType::DQBF && isQBF()) {
        convertToQBF();
        VLOG(2) << __FUNCTION__ << " formula is now a QBF after expanding " << stat(Statistics::UNIV_EXPANSION)
                << " variables.";
    }
 ...
 * \param lit a literal of the universal variable to be eliminated. The polarity
 * of `lit` determines which variable copy appears in which cofactor. \note This
 * function creates new variables which serve as copies of those existential
 * variables which depend upon `lit2var(lit)`. \todo If \f$x\f$ is the
 * eliminated universal variable, \f$y\f$ an existential one, and there is an
 * implication like \f$x\rightarrow y\f$, one can replace \f$y\f$ in the
 * 1-cofactor by 1. The remaining three cases are similar. We could look for
 * such implications using a SAT-solver or check if the implication is a blocked
 *       (binary clause).
 */
void
Formula::elimUVar(const Literal lit)
{
    enum Occurrence
    {
        NOT_AT_ALL = 0,
    ...
    VLOG_IF(!pseudo_deps.empty(), 2) << "Dependency scheme removed " << pseudo_deps.size() << " out of "
                                     << (lit_map.size() / 2) + pseudo_deps.size() << " dependencies.";
