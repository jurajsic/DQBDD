...
Literal
Formula::addAndGate(const Literal input1, const Literal input2)
{
...
Literal
Formula::addNandGate(const Literal input1, const Literal input2)
{
    return negate(addAndGate(input1, input2));
}

Literal
Formula::addOrGate(const Literal input1, const Literal input2)
{
    return negate(addAndGate(negate(input1), negate(input2)));
}

Literal
Formula::addNorGate(const Literal input1, const Literal input2)
{
    return addAndGate(negate(input1), negate(input2));
}

Literal
Formula::addXorGate(const Literal input1, const Literal input2)
{
    const Variable output_var = addEVar();
    _prefix->moveToRMB(output_var);
...
Literal
Formula::addMux(const Literal x0, const Literal x1, const Literal select)
{
    const Variable output_var = addEVar();
    _prefix->moveToRMB(output_var);
...
Literal
Formula::addAndGate(const std::vector<Literal>& inputs)
{
    const Variable output_var = addEVar();
    _prefix->moveToRMB(output_var);
...
Literal
Formula::addNandGate(const std::vector<Literal>& inputs)
{
    return negate(addAndGate(inputs));
}

Literal
Formula::addOrGate(const std::vector<Literal>& inputs)
{
    const Variable output_var = addEVar();
    _prefix->moveToRMB(output_var);

    Clause::ClauseData clause(2, 0);
    Gate               g(GateType::AND_GATE);  // or-gates are just and-gates with negated in-
                                               // and outputs!
    g._output_literal = var2lit(output_var, false);
    g._input_literals = inputs;
    g._encoding_clauses.reserve(inputs.size() + 1);
...
Literal
Formula::addNorGate(const std::vector<Literal>& inputs)
{
    return negate(addOrGate(inputs));
}

Literal
Formula::addXorGate(const std::vector<Literal>& inputs)
{
