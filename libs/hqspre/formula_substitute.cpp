...
Formula::substituteGate(const Gate& g)
{
    ...
    VLOG(3) << __FUNCTION__ << " Trying to substitute gate " << g;

    const auto approx_costs = computeSubstitutionCosts(g);
    if (approx_costs > _settings.max_substitution_cost + 200) {
        return false;
    }
    ...
    to_add.reserve(_occ_list[o_lit].size() * _occ_list[no_lit].size());
...
/**
 * \brief Computes the cost of replacing a Tseitin variable by its gate
 * definition.
 *
 * The costs are given as the number of literals by which the formula size
 * increases. Negative costs means that the formula becomes shorter. The value
 * is only an over-approximation as it does not take into account that
 * substitution may lead to tautological clauses or duplicate literals.
 *
 ...
 */
int
Formula::computeSubstitutionCosts(const Gate& g) const
{
    int result   = 0;
    int pos_cost = 0;
    int neg_cost = 0;
