    ...
    for (std::size_t i = 0; i < size; ++i) {
        ...
        if (sat_result == TruthValue::TRUE) {
            ...
            for (std::size_t j = i; j < size; ++j) {
                ...
                // We have seen both polarities of "check_var" -> we can remove this
 ...
 *
 * For all binary clauses which are not derivable from implications by
 * transitivity, a SAT-solver is called to check if they are implied by the
 * formula. If this is the case, the binary clause is added. \return true if the
 * formula was modified.
 */
bool
Formula::findImplicationsBySAT()
{
    VLOG(1) << __FUNCTION__;
    ...
    auto lit2index = [this](Literal lit1, Literal lit2) -> std::uint64_t {
        ...
        return (static_cast<std::uint64_t>(lit1) * static_cast<std::uint64_t>(maxLitIndex() + 1)
                + static_cast<std::uint64_t>(lit2));
    };
    ...
    for (Literal lit1 = minLitIndex(); lit1 < maxLitIndex(); ++lit1) {
        ...
        for (Literal lit2 = lit1 + 1; lit2 <= maxLitIndex(); ++lit2) {
            ...
            if (has_impl.first && has_impl.second) {
                VLOG(3) << __FUNCTION__ << ": " << lit2dimacs(lit1) << " implies " << lit2dimacs(lit2) << " and "
                        << lit2dimacs(negate(lit2)) << " => add unit " << lit2dimacs(negate(lit1));
            ...
            if (sat_result == TruthValue::FALSE) {
                ...
                VLOG(3) << __FUNCTION__ << "() add hidden implication " << lit2dimacs(assumptions[0]) << " "
                        << lit2dimacs(assumptions[1]);
 ...
 */
void
Formula::checkUNSAT(const std::size_t num_random_patterns)
{
    VLOG(1) << __FUNCTION__ << " using " << macro_print(SatSolver) << " as SAT solver.";
