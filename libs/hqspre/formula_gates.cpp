...
/**
 * \brief Checks if two clauses contain the same variables (with different
 * polarities). \pre Requires c1 and c2 to be sorted. \pre Clauses must not be
 * tautological or contain duplicate literals.
 */
bool
...
Formula::determineGates(const bool and_gates, const bool xor_gates, const bool mux_gates, const bool semantic_gates)
{
    ...
    std::size_t count = 0;
    if (and_gates) {
        count += findANDGates(gateDep, _settings.extend_gates);
    }
    if (xor_gates) {
        count += findXORGates(gateDep);
    }
    if (mux_gates) {
        count += findMUXGates(gateDep);
    }
    if (semantic_gates) {
        count += findSemanticGates(gateDep);
    }
...
Formula::findEquivalentGates()
{
    ...
    for (const auto& entry : lit_map) {
        if (isPositive(entry.first) && entry.first != entry.second) {
            VLOG(3) << "Gate outputs " << lit2dimacs(entry.first) << " and " << lit2dimacs(entry.second)
                    << " are equivalent.";
            replaceLiteral(entry.first, entry.second);

#ifdef SKOLEM
            _skolem_data.push_back(
                std::make_unique<SkolemEquiv>(lit2var(entry.second), negate_if(entry.first, isNegative(entry.second))));
#endif
            ++count;
        }
    }
 ...
 */
static bool
find_c1(const Clause& c, const Literal out, Literal& sel, Literal& in1)
{
 ...
 */
static bool
find_c3(const Clause& c, const Literal sel, const Literal in1, const Literal out, Literal& in0)
{
    ...
    for (std::size_t i = 0; i < 3; ++i) {
        if (c[i] == sel) {
            found_sel = true;
        } else if (c[i] == negate(out)) {
            found_out = true;
        } else if (c[i] == in1 || c[i] == negate(in1)) {
            return false;
        } else {
            in0 = c[i];
        }
 ...
 */
std::size_t
Formula::findMUXGates(std::vector<std::vector<Variable>>& gateDep)
{
...
bool
Formula::isGateOutput(const Variable var) const noexcept
{
