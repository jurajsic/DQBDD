 ...
 * \return true iff the literal is assigned false, false if it is true or unknown.
 */
inline static bool isUnsatisfied(const Literal lit, const std::vector<TruthValue>& assignment)
{
 ...
 * \return true iff the literal is assigned true, false if it is false or unknown.
 */
inline static bool isSatisfied(const Literal lit, const std::vector<TruthValue>& assignment)
{
...
/**
 * \brief Inserts a literal (and all its direct implications) into the queue of literals that are to be propagated.
 *
 * This function also handles the binary clauses (i.e., implications).
 *
 * \throws ConflictException if a conflict occurs (literal was already assigned in the opposite way)
 * \param lit the literal to be enqueued
 * \param the ID of the clause which should be ignored (used e.g. in vivification)
 */
void SatPropagator::enqueue(const Literal lit, const int except_clause)
{
    const Variable var = lit2var(lit);
    const bool already_set = (_assignment[var] != TruthValue::UNKNOWN);

    if (isPositive(lit)) {
        if (_assignment[var] == TruthValue::FALSE) throw ConflictException();
        else _assignment[var] = TruthValue::TRUE;
    } else {
        // literal is negative
        if (_assignment[var] == TruthValue::TRUE) throw ConflictException();
        else _assignment[var] = TruthValue::FALSE;
    }

    if (already_set) return;

   _unit_stack.push_back(lit);
    for (std::size_t ptr = _unit_stack.size() - 1;
        ptr != _unit_stack.size();
        ++ptr)
    {
        ...
        for (const auto bin: _implications[current_lit]) {
            ...
            } else if (isUnsatisfied(implied_lit, _assignment)) throw ConflictException();
        }
    }
}
 ...
 */
void SatPropagator::createWatches()
{
    _watches.clear();
 ...
 */
void SatPropagator::createWatches(const ClauseID c_nr)
{
    const Clause& clause = _clauses[c_nr];
 ...
 *
 * This function searches for all unit literals which result from assigning the given literal.
 * The found implications are stored in `_units`, their satisfying assignment in `_assignment`.
 * If a conflict occurs, i.e., two contradictory literals are found, a ConflictException is raised.
 * The parameter `except_clause` can be used to ignore a certain clause of the database; this
 * is necessary, e.g., for vivification where the vivified clause is ignored.
 *
 ...
 */
void SatPropagator::bcp(const Literal assumption, const int except_clause)
{
    val_assert(_prefix);
    ...
    while (!_unit_stack.empty()) {
        ...
        for (std::size_t current_pos = 0; current_pos < num_watches; ++current_pos) {
            ...
            // Take care that lit1 is the literal over whose watch list we are running;
            // pos1 is its position in the current clause. The other watch literal is lit2
            // with position pos2.
            std::size_t pos1 = 0;
            std::size_t pos2 = 0;
            Literal lit2  = 0;
            ...
            if (isSatisfied(lit2, _assignment)) {
                // We have to keep the watch entry -> copy it from current_pos to next_free
                _watches[lit1][next_free] = c_nr;
                ++next_free;
                continue;
            }
            ...
            if (new_pos1 == pos1) {
                // We could not find a replacement for the first watch -> second watch is unit (or unsatisfied as well).

                try {
                    enqueue(lit2, except_clause);
                } catch (const ConflictException& e) {
                    // We need to leave the _watches structure in a consistent state.
                    // That means we have to copy the remaining entries from 'current_pos' on to 'next_free'
                    // and resize _watches[lit1]. Then re-throw the exception.
                ...
                // We have to keep the current watch entry -> copy it from current_pos to next_free
                _watches[lit1][next_free] = c_nr;
                ++next_free;
                continue;
            } else {
                // We have to update the first watch literal entry; the entry in _watches[lit1] is automatically deleted.
...
/**
 * \brief Clears the unit literals found in previous calls to SatPropagator::bcp(Literal, int).
 *
 * Afterward, all literals are unassigned again such that new BCP operations can be executed.
 */
void SatPropagator::clear()
{
 ...
 *
 * This function must be called <i>after</i> Formula::addClause() with the clause's ID.
 */
void SatPropagator::addClause(const ClauseID c_nr)
{
 ...
 */
void SatPropagator::removeClause(const ClauseID c_nr)
{
 ...
 * \return true if the watch literal datastructure is consistent; false if an error was detected.
 */
bool SatPropagator::checkConsistency() const
{
    if (!_prefix) {
    ...
    for (ClauseID c_nr = 0; c_nr < _clauses.size(); ++c_nr) {
        ...
        if (std::find(_watches[lit1].cbegin(), _watches[lit1].cend(), c_nr) == _watches[lit1].cend()) {
            LOG(ERROR) << "Clause no. " << c_nr << " not in watch list of its first watched literal " << lit2dimacs(lit1) << '.';
 ...
 * \brief Inserts a literal (and all its direct implications) into the queue of literals that are to be propagated.
 *
 * This function also handles the binary clauses (i.e., implications) and applies
 * universal reduction to the binary clauses.
 *
 * \throws ConflictException if a conflict occurs (literal was already assigned in the opposite way)
 * \param lit the literal to be enqueued
 * \param the ID of the clause which should be ignored (used e.g. in vivification)
 */
void QbfPropagator::enqueue(const Literal lit, const int except_clause)
{
    const Variable var = lit2var(lit);
    const bool already_set = (_assignment[var] != TruthValue::UNKNOWN);

    if (isUnsatisfied(lit, _assignment)) throw ConflictException();
    else _assignment[var] = makeSatisfied(lit);
    ...
    if (!already_set) {
            ...
            ++ptr)
        {
            ...
            for (const auto bin: _implications[current_lit]) {
                ...
                if (_assignment[implied_var] == TruthValue::UNKNOWN) {
                    if (_prefix->isUniversal(implied_var)) throw ConflictException();
                    else {
                        _assignment[implied_var] = makeSatisfied(implied_lit);
                        _unit_stack.push_back(implied_lit);
                    }
 ...
 */
void QbfPropagator::createWatches()
{
    _watches.clear();
    _watches.resize(_prefix->maxLitIndex() + 1);
    _watchpos.assign(_clauses.size(), std::make_pair(0, 0));
    ...
    for (ClauseID c_nr = 0; c_nr < _clauses.size(); ++c_nr) {
        ...
        if (clause.getStatus() != ClauseStatus::DELETED && clause.size() > 2) {
            ...
            if (only_exist) SatPropagator::createWatches(c_nr);
            else createUnivWatches(c_nr);
        }
    }
...
void QbfPropagator::createUnivWatches(const ClauseID c_nr)
{
}
