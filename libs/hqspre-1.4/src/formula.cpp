 ...
 * \param check_subsumption if true, it is checked whether the clause subsumes other clauses in database
 * \param status Specifies if the clause is mandatory or optional (deleted does not make much sense here!)
...
bool Formula::removeLiteral(const ClauseID c_nr, const Literal lit)
{
    ...
    if (clause.size() == 2) {
        ...
        if (clause[0] == lit) pushUnit(clause[1], PureStatus::UNIT);
        else pushUnit(clause[0], PureStatus::UNIT);
        removeClause(c_nr);
        return true;
    }
 ...
 */
void Formula::printSettings() const
{
    LOG(INFO) << _settings << '\n';
}
