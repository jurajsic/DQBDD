 ...
 * \note The clause will not be extended; it is only checked whether universal literals can be added which are blocked literals
...
bool Formula::removeBlockedAndFriends()
{
    ...
    while (!_blocked_candidates.empty()) {
        ...
        if (_clauses[c_nr].size() < _settings.min_bce_size) { continue; }
...
bool Formula::addHiddenLiterals(const int c_nr, Clause::ClauseData& clause, std::uint64_t& sign) const
{
    ...
    for (std::size_t c = 0; c != csize; ++c)
    {
        ...
        for (std::size_t j = 0; j != _occ_list[cur_lit].size(); ++j ) {
            ...
            // Clause "other" can never contain a hidden literal if it is larger or has the same size as current clause
 ...
 * \param sign the signature of the clause passed to this function
 * \note The passed clause is modified together with its signature
 * \pre Assumes that "_seen" vector was initialized with "clause"
 * \return true if clause is a hidden tautology
 */
