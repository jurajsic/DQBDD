// $Id: milp_solver.hpp 2010 2018-02-12 18:20:01Z wimmer $
...
namespace hqspre {
    ...
    public:
        using VarType = int;
        enum class VarSort { INT, REAL };
        enum class ObjectiveType { MINIMIZE, MAXIMIZE };

        MilpSolver() = default;
        virtual ~MilpSolver() = default;

        virtual VarType addVariable(VarSort sort,
                                    bool lower_bounded,
                                    double lower_bound,
                                    bool upper_bounded,
                                    double upper_bound) = 0;

        virtual void addConstraint(const std::vector<VarType>& variables,
        ...
        virtual void setObjective(const std::vector<VarType>& variables,
    ...
    public:
        ...
        VarType addVariable(VarSort sort,
                            bool lower_bounded,
                            double lower_bound,
                            bool upper_bounded,
                            double upper_bound) override;

        void addConstraint(const std::vector<VarType>& variables,
                           const std::vector<double>& coeffs,
                           bool lower_bounded,
                           double lower_bound,
                           bool upper_bounded,
        ...
        void setObjective(const std::vector<VarType>& variables,
