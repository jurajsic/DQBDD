 ...
 */
void Formula::pushUnit(const Literal lit, const PureStatus status)
{
    ...
    if (status == PureStatus::PURE) ++stat(Statistics::PURE);
    ...
    while (!_unit_stack.empty()) {
        ...
        for (const ClauseID c_nr: _occ_list[neg_top_lit]) {
            ...
            if (c.size() == 2) {
                ...
                if (c[0] == neg_top_lit) pushUnit(c[1], PureStatus::UNIT);
                else pushUnit(c[0], PureStatus::UNIT);
